...
import {IAuth} from '@pandit-abhishek1/sharedservices'
...
class AuthService{
  ...
  async signUp(body: IAuth): Promise<AxiosResponse> {
    console.log('auth.services signup body: ', body, `${config.AUTH_BASE_URL}/api/v1/auth`);
    const response: AxiosResponse = await this.axiosService.axios.post('/signup', body);
    console.log('auth.services signup response: ', response);
    return response;
  }
